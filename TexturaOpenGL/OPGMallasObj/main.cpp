/*
La estructura de datos que ustilizamos para almacenar las mallas fue
...
protected:
   ...
   bool mxiz, mxdr, myup, mydw, mzad, mzat;
...
public:
    ...
    void initialize_textures(void)
    {
        ...
        FIBITMAP* bitmap = FreeImage_Load(
            FreeImage_GetFileType("Mallas/luna301.jpg", 0),
            "Mallas/luna301.jpg");  //*** Para Textura: esta es la ruta en donde se encuentra la textura
	...
	virtual void OnRender(void)
	{
      ...
      if (shader) shader->begin();
         ...
         /*glPushMatrix();
            glScalef(2, 2, 2);
            objs3D[2]->DibujarMalla(0.5, 1.0, 0);
         glPopMatrix();*/
         glPushMatrix();
            glScalef(5, 5, 5);
            objs3D[2]->DibujarMalla(-0.25, 0.125, 0);
         glPopMatrix();
         glPushMatrix();
            glScalef(4, 4, 4);
            objs3D[3]->DibujarMalla(-0.30, 0.008, 0);
         glPopMatrix();
         glPushMatrix();
            glScalef(3, 3, 3);
            objs3D[4]->DibujarMalla(0.1 , 0.08, 0);
         glPopMatrix();
	...
	virtual void OnInit()
	{
      ...
      if (!objmodel_ptr1)
      {
          objmodel_ptr1 = glmReadOBJ("Mallas/luna.obj");
          if (!objmodel_ptr1)
              exit(0);
      ...
      time0 = clock();
      timer010 = 0.0f;
      mxiz, mxdr, myup, mydw, mzad, mzat = false;
      movX = 0.5;
      movY = 1.0;
      movZ = 0.0;
